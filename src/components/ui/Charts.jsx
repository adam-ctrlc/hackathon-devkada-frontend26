...
export function Ring({ value = 72, size = 120, stroke = 10, label, sub, tone = "brand" }) {
  ...
  const labelSize  = size >= 100 ? "text-[26px]" : size >= 70 ? "text-[18px]" : "text-[13px]";
  ...
  return (
    <div className="relative inline-grid place-items-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 grid place-items-center text-center pointer-events-none">
        <div>
          <div className={`font-display leading-none text-slate-900 ${labelSize}`}>
            {label ?? value}
          </div>
          {sub && (
...
export function Spark({ data = [], w = 120, h = 36, tone = "brand", area = true }) {
  const gradId = useId();
